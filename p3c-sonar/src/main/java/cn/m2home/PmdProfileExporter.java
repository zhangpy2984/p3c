...
import cn.m2home.xml.PmdProperty;
import cn.m2home.xml.PmdRule;
import cn.m2home.xml.PmdRuleset;
...
public class PmdProfileExporter extends ProfileExporter {
    public PmdProfileExporter() {
        super(CustomRulesDefinition.REPOSITORY_KEY, CustomRulesDefinition.PLUGIN_NAME);
    ...
    public void exportProfile(RulesProfile profile, Writer writer) {
        ...
        PmdRuleset tree = createPmdRuleset(CustomRulesDefinition.REPOSITORY_KEY, profile.getActiveRulesByRepository(CustomRulesDefinition.REPOSITORY_KEY), profileName);
    ...
    static void processXPathRule(String sonarRuleKey, PmdRule rule) {
        if (CustomRulesDefinition.XPATH_CLASS.equals(rule.getRef())) {
            rule.setRef(null);
            PmdProperty xpathMessage = rule.getProperty(CustomRulesDefinition.XPATH_MESSAGE_PARAM);
            if (xpathMessage == null) {
                throw new IllegalArgumentException("Property '" + CustomRulesDefinition.XPATH_MESSAGE_PARAM + "' should be set for PMD rule " + sonarRuleKey);
            }
            rule.setMessage(xpathMessage.getValue());
            rule.removeProperty(CustomRulesDefinition.XPATH_MESSAGE_PARAM);
            PmdProperty xpathExp = rule.getProperty(CustomRulesDefinition.XPATH_EXPRESSION_PARAM);
            if (xpathExp == null) {
                throw new IllegalArgumentException("Property '" + CustomRulesDefinition.XPATH_EXPRESSION_PARAM + "' should be set for PMD rule " + sonarRuleKey);
            }
            xpathExp.setCdataValue(xpathExp.getValue());
            rule.setClazz(CustomRulesDefinition.XPATH_CLASS);
